...
class NamespacedTreeBuilder(etree.TreeBuilder):
    def __init__(self, *args, **kwargs):
        ...
        self.__ns = None
        self.__opened = []
        etree.TreeBuilder.__init__(self, *args, **kwargs)
...
class Saml2Metadata:
    ...
    def generate_services(self, map, options, listing):
        ...
        endpoint_idx = collections.defaultdict(int)
    ...
    def __str__(self):
        return '<?xml version="1.0"?>\n' + etree.tostring(self.root_element()).decode('utf-8')
...
def authnresponse_checking(login, subject_confirmation, logger, saml_request_id=None):
    ...
    logger.debug(
        'assertion validity timeslice respected %s <= %s < %s' % (not_before, str(now), not_on_or_after)
    )
    return True
    ...
    for att_statement in assertion.attributeStatement:
        for attribute in att_statement.attribute:
            ...
            try:
                ...
                logger.warning(
                    'get_attributes_from_assertion: error decoding name of attribute %s' % attribute.dump()
                )
            else:
                try:
